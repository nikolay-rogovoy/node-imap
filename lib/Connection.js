...
class Connection extends EventEmitter {
  ...
  connect() {
    ...
    parser.on('body', function (stream, info) {
      ...
      if (thisbody instanceof Array) {
        thisbody.sort();
        for (let thisbodyItem of thisbody) {
          if (thisbodyItem instanceof Array) {
            thisbodyItem.sort();
          }
        }
      }
      for (let i = 0, len = toget.length; i < len; ++i) {
        if (toget[i] instanceof Array) {
          toget[i].sort();
          for (let togetItem of toget[i]) {
            if (togetItem instanceof Array) {
              togetItem.sort();
            }
          }
        }
        console.log('_deepEqual:', thisbody, toget[i]);
